...
class ThreeStagePipeline:
    ...
    def __init__(self,
        ...
        # Initialize local model pipeline (may fail on systems without GPU).
        try:
            self.stage1_pipeline = MultiLayerPipeline(
                peft_model_path=peft_model_path
            )
            print("   ✅ Stage 1 local model ready")
        except Exception as e:
            print(f"   ⚠️  Stage 1 local model error: {e}")
            self.stage1_pipeline = None

        # Defer RAG init (heavy embeddings) to first use for faster startup.
        print("   ℹ️  RAG retriever will initialize on first query (lazy load)")
    ...
    def _ensure_rag(self):
        """Initialize RAG on-demand to avoid heavy startup time."""
        if self.rag is not None or self._rag_init_attempted:
            return
        self._rag_init_attempted = True
        try:
            print("   Initializing RAG retriever (first use)...")
...
COMBINED AND FORMATTED ANSWER:"""
    ...
    def generate_answer(self, question: str, use_formatter: bool = True) -> Dict:
        ...
        # Stage 1: RAG retrieval + optional local model
        stage1_start = time.time()
        stage1_result = None
        context = ""
        references = []

        # Retrieve context even if local model is unavailable.
        retrieved_docs = []
        self._ensure_rag()
        if self.rag:
            try:
        ...
        if self.stage1_pipeline:
            try:
                ...
                # Use retrieved references if pipeline didn't provide them
                if stage1_result and not stage1_result.get('references'):
                    stage1_result['references'] = references
            except Exception as e:
                print(f"   ⚠️  Stage 1 generation error: {e}")
                stage1_result = {'answer': '', 'error': str(e)}
        else:
            stage1_result = {'answer': '', 'references': references}
