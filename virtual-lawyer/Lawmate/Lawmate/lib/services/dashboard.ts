...
// API Functions
export async function getCitizenDashboard(userId?: string): Promise<CitizenDashboardData> {
  console.log("📡 Calling /api/dashboard/citizen...")
  try {
    const qs = userId ? `?user_id=${encodeURIComponent(userId)}` : ""
    const data = await api.get<CitizenDashboardData>(`/api/dashboard/citizen${qs}`);
...
export async function getLawyerDashboard(lawyerId?: string): Promise<LawyerDashboardData> {
  console.log("📡 Calling /api/dashboard/lawyer...")
  try {
    const qs = lawyerId ? `?lawyer_id=${encodeURIComponent(lawyerId)}` : ""
    const data = await api.get<LawyerDashboardData>(`/api/dashboard/lawyer${qs}`);
